...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub Perform(NewRequest As HTTP.Request)
		  ...
		  Sock.Port = p
		  If sock.Secure Then
		    ResponseMain1.Log("Attempting a secure connection to " + CurrentRequest.Path.ToString + " on port " + Format(p, "#####0"), 1)
		  Else
		    ResponseMain1.Log("Attempting a connection to " + CurrentRequest.Path.ToString + " on port " + Format(p, "#####0"), 1)
		  End If
		  Sock.Connect()
