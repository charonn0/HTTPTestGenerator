...
Inherits TCPSocket
	...
	#tag Method, Flags = &h21
		Private Function DefaultRequestHandler(HTTPVerb As String, HTTPPath As String, HTTPVersion As Single, PostContent As String) As String
		  ...
		  If TamperRequestHeaders(tamper) Then
		    ...
		    QueryHeaders = tamper
		  End If
		  PrintRequest(HTTPVerb + " " + HTTPPath + " HTTP/" + Format(HTTPVersion, "0.0") + EndOfLine + QueryHeaders.Source)
		  ...
		  Case "OPTIONS"
		    Dim f As FolderItem = FindItem(HTTPPath)
		    HTTPreply = HTTPResponse(200) + CRLF 'HTTP 200 OK
		    ReplyHeaders = ResponseHeaders()
		    If f <> Nil And f.Exists Then
		      ReplyHeaders.AppendHeader("Allow", "GET, HEAD, OPTIONS")
		    End If
		    If f <> Nil And Not f.Directory Then
		      ReplyHeaders.AppendHeader("Accept-Ranges", "bytes")
		    End If
		    mLastHTTPCode = 200
		    pagedata = ""
		  ...
		  Case "TRACE"
		    HTTPreply = HTTPResponse(200) + CRLF 'HTTP 200 OK
		    ReplyHeaders = ResponseHeaders(0, "message/http")
		    mLastHTTPCode = 200
		    pagedata = HTTPVerb + " " + HTTPPath + " HTTP/" + Format(HTTPVersion, "#.0") + CRLF + QueryHeaders.Source + CRLF + CRLF + PostContent
		    
		  Case "POST", "PUT", "DELETE", "CONNECT", "PATCH"
		    'HTTP 405 Method Not Allowed
		    ReplyHeaders.AppendHeader("Allow", "GET, HEAD, TRACE, OPTIONS")
	...
	#tag Method, Flags = &h1
		Protected Sub DoResponse(PageData As String)
		  RaiseEvent PrintResponse(pagedata)
		  Me.Write(PageData)
		End Sub
	#tag EndMethod
	...
	#tag Hook, Flags = &h0
		Event PrintRequest(Request As String)
	#tag EndHook

	#tag Hook, Flags = &h0
		Event PrintResponse(Response As String)
	#tag EndHook
