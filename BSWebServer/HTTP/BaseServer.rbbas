...
Inherits ServerSocket
	...
	#tag Method, Flags = &h0
		Sub AddRedirect(Page As HTTP.Response)
		  ...
		  Me.Log(CurrentMethodName + ": " + page.Path.Path, Log_Trace)
		  Redirects.Value(page.Path.Path) = Page
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Function CheckRedirect(ClientRequest As HTTP.Request, Session As HTTP.Session) As HTTP.Response
		  ...
		  Wend
		  Dim redir As HTTP.Response = GetRedirect(Session, clientrequest.Path.Path)
		    ...
		    Try
		      ...
		      If clientrequest.HasHeader("Content-Type") Then
		        ...
		        If InStr(type, "multipart/form-data") > 0 Then
		          ...
		          clientrequest.MultiPart = MultipartForm.FromString(clientrequest.MessageBody, boundary)
		        End If
		      End If
