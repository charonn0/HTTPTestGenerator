	...
	#tag Event
		Function HandleRequest(ClientRequest As HTTP.Request) As HTTP.Response
		  ...
		  Case RequestMethod.GET, RequestMethod.HEAD
		    Dim item As FolderItem = FindItem(ClientRequest.Path.Path)
		    ...
		    Else
		      ...
		      If item.Directory Then
		        ...
		        doc = New DirectoryIndex(item, ClientRequest.Path.ToString)
		        doc.StatusCode = 200
		        WebServer.DirectoryIndex(doc).Populate
		      ElseIf ClientRequest.HasHeader("Range") Then
		        doc = GetFileResponse(item, ClientRequest.Path.ToString, ClientRequest.RangeStart, ClientRequest.RangeEnd)
		      Else
		        doc = GetFileResponse(item, ClientRequest.Path.ToString)
		      End If
		    End If
		  End Select
